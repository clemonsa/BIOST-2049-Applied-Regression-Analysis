...
We will fit elastic net family regression with `library(glmnet)`. `library(glmnetUtils)` adds some quality of life feature to glmnet, such as using formula ~ syntax. 
```{r, message=F}
library(glmnet)
library(glmnetUtils)
...
To plot the curve for a specific $\alpha$, the fitted model can be pulled out by index. At index 1 is the ridge fit with $\alpha$ = 0.

```{r}
# List of alpha
cv_fit_el$alpha
# Get the first alpha = 0, ridge
plot(cv_fit_el$modlist[[1]])
```

Generic methods can be applied to these models. For coefficients, the method `coef()` can be used. (`?coef.cva.glmnet` and `?coef.cv.glmnet`)

```{r}
methods(class = class(cv_fit_el)[2])
# Lasso fit
coef(cv_fit_el, alpha = 1, s = "lambda.min")
```

```{r}
# Ridge fit
coef(cv_fit_el, alpha = 0, s = "lambda.min")
```
...
```{r}
plot(log(cv_fit_el$modlist[[1]]$lambda), cv_fit_el$modlist[[1]]$glmnet.fit$dev.ratio,
     xlab = expression(log(lambda)), ylab = expression(R^2))
```
